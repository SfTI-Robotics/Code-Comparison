...
def simulate():
    ...
    for episode in range(NUM_EPISODES):
        ...
        for t in range(MAX_T):
            ...
            #  amax: return max along array or axis
...
def get_explore_rate(t):
    # updates with exponential decay, to slowly be more greedy
    return min(1, MIN_EXPLORE_RATE*NUM_EPISODES/(t+1))

def get_learning_rate(t):
    # learning rate updates with exponential growth, so you don't keep the 'bad habits'
    return min(0.8, MIN_LEARNING_RATE*NUM_EPISODES/(t+1))


def angle_to_bucket(angle):
    # finds the max out of lower bound and angle, then compares to upper bound to return the minimum one
    #  ensures angle is in bounds
    angle = min(ANGLE_BOUND[1], max(ANGLE_BOUND[0], angle))
    return int((angle - ANGLE_BOUND[0])/(ANGLE_BOUND[1] - ANGLE_BOUND[0])*(NUM_STATE-1))
    # gets ratio of (difference between angle and lower bound) / (bound range) 
    # then multiply with 9 (since bucket-states are from 0 -> 9, has 10 states)


def bucket_to_angle(state):
    # state 0 = lower bound, state 9 = upper bound
